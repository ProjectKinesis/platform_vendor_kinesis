...
func init() {
	android.RegisterModuleType("kinesis_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = kinesisExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = kinesisExpandVariables(ctx, dep_file)
	...
	cmd := kinesisExpandVariables(ctx, String(g.properties.Cmd))
