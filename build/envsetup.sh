function __print_kinesis_functions_help() {
cat <<EOF
Additional Project Kinesis functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- kinesisgerrit:   A Git wrapper that fetches/pushes patch from/to Project Kinesis Gerrit Review.
- kinesisrebase:   Rebase a Gerrit change and push it again.
- kinesisremote:   Add git remote for Project Kinesis Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for Project Kinesis Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the kinesis model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch kinesis_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/kinesis-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.kinesis.device | grep -q "$KINESIS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $KINESIS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function kinesisremote()
{
    ...
    git remote rm kinesis 2> /dev/null
    ...
    then
        ...
        local PFX="Project Kinesis/"
    else
        local PROJECT=$REMOTE
    fi

    local KINESIS_USER=$(git config --get review.review.projectkinesis.org.username)
    if [ -z "$KINESIS_USER" ]
    then
        git remote add kinesis ssh://review.projectkinesis.org:29418/$PFX$PROJECT
    else
        git remote add kinesis ssh://$KINESIS_USER@review.projectkinesis.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'kinesis' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/ProjectKinesis/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.kinesis.device | grep -q "$KINESIS_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        kinesisremote
        git push kinesis HEAD:refs/heads/'$1'
    fi
    '
}

function kinesisgerrit() {
    ...
    local user=`git config --get review.review.projectkinesis.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "kinesisgerrit" ]; then
...
function kinesisrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Project Kinesis Gerrit Rebase Usage: "
        echo "      kinesisrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.projectkinesis.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.kinesis.device | grep -q "$KINESIS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/kinesis/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $KINESIS_FIXUP_COMMON_OUT ]; then
